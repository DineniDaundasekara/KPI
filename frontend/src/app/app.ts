...
import { Router, NavigationEnd, NavigationError, Event as RouterEvent } from '@angular/router';
...
export class App {
  ...
  ngOnInit(): void {
    ...
    // Global runtime error handlers to capture issues that break rendering
    window.addEventListener('error', (evt: ErrorEvent) => {
      try {
        this.lastError = evt.message + (evt.error ? ' | ' + (evt.error.stack || String(evt.error)) : '');
      } catch {
        this.lastError = String(evt.message || 'Unknown error');
      }
    });
    window.addEventListener('unhandledrejection', (evt: PromiseRejectionEvent) => {
      try {
        const reason = (evt.reason && (evt.reason.stack || evt.reason.message)) || String(evt.reason);
        this.lastError = 'UnhandledRejection: ' + reason;
      } catch {
        this.lastError = 'UnhandledRejection';
      }
    });
